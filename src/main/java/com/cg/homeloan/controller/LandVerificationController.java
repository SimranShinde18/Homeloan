...
public class LandVerificationController {
	@Autowired
    private ILandVerificationService landVerificationService;
	
	@PutMapping(value = "/landstatus",  produces = {"application/json","application/xml"},consumes =  {"application/json","application/xml"})
    public ResponseEntity<LoanApplication> updateStatus( @RequestBody LoanApplication loanApplication) {
		return new ResponseEntity<>(landVerificationService.updateStatus(loanApplication), HttpStatus.OK);
    }
}
